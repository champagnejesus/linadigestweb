...
import { databaseErrorMessage } from "../../../../db/supabase";
...
export async function POST(request: Request) {
  try {
    ...
    if (password.length < 8 || password.length > 100) return Response.json({ error: "La nueva clave debe tener al menos 8 caracteres" }, { status: 400 });
    if (password !== confirmation) return Response.json({ error: "Las claves no coinciden" }, { status: 400 });
    if (!/[A-Za-zÁÉÍÓÚáéíóúÑñ]/.test(password) || !/\d/.test(password)) return Response.json({ error: "Usa al menos una letra y un número" }, { status: 400 });
