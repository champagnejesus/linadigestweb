...
export default function ScannerView({
  ...
  onScanned: (stock: number, outputDelta?: number) => void;
  onBarcodeSaved: () => void;
}) {
  ...
  const [lastUndoableScan, setLastUndoableScan] = useState<UndoableScan | null>(null);
  ...
  const processScan = useCallback(async (item: QueueItem) => {
    try {
      ...
      if (!result.duplicate) {
        ...
        setLastUndoableScan({ id: item.id });
  ...
  async function undoLastScan() {
    if (!lastUndoableScan || undoing || processing) return;
    setUndoing(true);
    try {
      const response = await fetch("/api/inventory/scan/undo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ scanId: lastUndoableScan.id }),
      });
      const result = (await response.json()) as { error?: string; stock?: number };
      if (!response.ok || typeof result.stock !== "number") {
        throw new Error(result.error || "No fue posible deshacer la lectura");
      }
      setSessionScans((value) => Math.max(0, value - 1));
      setLastUndoableScan(null);
      onScanned(result.stock, 0);
      setLastResult({
        kind: "idle",
        title: "Última lectura anulada",
        detail: `La unidad volvió al inventario · Stock actual: ${result.stock.toLocaleString("es-CL")}`,
      });
      playFeedback(true);
    } catch (error) {
      setLastResult({
        ...
        detail: error instanceof Error ? error.message : "Inténtalo nuevamente.",
      });
      playFeedback(false);
    } finally {
      setUndoing(false);
  ...
  function enqueueScan(event: FormEvent) {
    ...
    if (
      ...
      && now - lastEnqueuedScan.current.at < 300
    ) {
      setLastResult({
        kind: "error",
    ...
    <section className="scanner-section">
      ...
      <div className="scanner-layout">
          ...
          {canScan ? (
            <>
              <form className="scanner-form" onSubmit={enqueueScan}>
                ...
                <div className="scanner-input-wrap">
                  ...
                  <input
                    ...
                    onBlur={() => window.setTimeout(() => scannerInput.current?.focus(), 80)}
                  />
                  <button type="submit">Descontar 1</button>
                </div>
                <p id="scanner-help">El lector escribe el código y envía Enter. Cada pistoleo confirmado descuenta automáticamente 1 unidad.</p>
              </form>
              ...
              <div className={`scan-feedback ${lastResult.kind}`} aria-live="polite">
                ...
                <div className="scan-feedback-copy"><strong>{lastResult.title}</strong><p>{lastResult.detail}</p></div>
                {lastUndoableScan && !processing && (
                  <button className="scan-undo-button" type="button" onClick={() => void undoLastScan()} disabled={undoing}>
                    {undoing ? "Anulando…" : "Deshacer última"}
                  </button>
                )}
              </div>
        ...
        <aside className="scanner-summary">
          ...
          <BarcodeLabelPrinter value={linkedBarcode} productName={product.name} />
...
function BarcodeSetup({
  ...
  const generatedBarcode = product.sku.trim().toUpperCase();
  ...
  async function generateAndLink() {
    ...
    try {
      const response = await fetch("/api/inventory/barcode", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ productId: product.id, barcode: generatedBarcode }),
      });
      ...
      <article className="panel barcode-setup-card">
        ...
        {canManage ? (
          <>
            ...
            {currentBarcode ? (
              <>
                <h3>Código LinaDigest vinculado</h3>
                <p>El producto ya está vinculado. Cada vez que presiones <strong>Imprimir etiqueta</strong>, LinaDigest imprimirá una sola etiqueta con el código universal del producto.</p>
                <BarcodeLabelPrinter value={currentBarcode} productName={product.name} />
                {onCancel && <button className="text-button barcode-return-button" type="button" onClick={onCancel}>Volver al lector</button>}
              </>
            ) : (
              <>
                <h3>Genera y vincula el código desde aquí</h3>
                <p>La página creará un <strong>Code 128</strong>, lo asociará a <strong>{product.name}</strong> y dejará lista la etiqueta para imprimir.</p>
                <div className="generated-barcode-value" aria-label={`Código que se generará: ${generatedBarcode}`}>
                  <span>CÓDIGO INTERNO</span>
                  <strong>{generatedBarcode}</strong>
                  <small>Código base para identificar el producto</small>
                </div>
                <button
                  className="button primary barcode-generate-button"
                  type="button"
                  onClick={() => void generateAndLink()}
                  disabled={saving}
                >
                  <span aria-hidden="true">▥</span>
                  {saving ? "Generando y vinculando…" : "Generar y vincular código"}
                </button>
                <p className="barcode-generate-help">Después podrás imprimir una etiqueta por vez en la Brother QL-800 y cada lectura descontará 1 unidad.</p>
          ...
          <>
            ...
            <h3>Falta generar el código de LinaDigest</h3>
            <p>Miguel Angel o Daniela Vasquez deben entrar una sola vez y presionar <strong>Generar y vincular código</strong>.</p>
          </>
        )}
      </article>
    </section>
