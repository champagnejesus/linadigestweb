...
export default function InventoryApp({ session }: { session: { name: string; username: string; mustChangePassword: boolean } }) {
  ...
  async function signOut() {
    ...
    try {
      ...
      setToast(signOutError instanceof Error ? signOutError.message : "No fue posible cerrar la sesión");
      setSigningOut(false);
    }
  }
      ...
      <aside className="sidebar">
        <div className="brand">
          <div className="brand-mark">
            <Image src="/linadigest-logo-corporativo-v2.png" alt="LinaDigest" width={58} height={52} priority unoptimized />
          </div>
          <div>
            ...
            <>
              ...
              {activeTab === "escaner" && (
                <ScannerView
                  ...
                  onScanned={(stock, outputDelta = 1) => {
                    setData((current) => current ? {
                      ...current,
                      product: { ...current.product, stock },
                      stats: { ...current.stats, outputs: Math.max(0, current.stats.outputs + outputDelta) },
                    } : current);
      ...
      <section className="dashboard-grid">
        <article className="panel product-panel">
          ...
          <div className="product-overview">
            <div className="product-picture"><Image src="/linadigest-logo-corporativo-v2.png" alt="Logo de LinaDigest" width={168} height={148} unoptimized /></div>
    ...
    <section>
      ...
      <div className="panel inventory-table-wrap">
        ...
        <div className="inventory-row">
          <div className="inventory-product"><Image src="/linadigest-logo-corporativo-v2.png" alt="LinaDigest" width={58} height={52} unoptimized /><div><strong>{data.product.name}</strong><span>{data.product.sku} · {data.product.unit}</span></div></div>
